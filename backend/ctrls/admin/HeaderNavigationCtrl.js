...
const fetchById = async (req, res) => {
  const { id } = req.params;
  const navigation = await HeaderNavigation.findById(id);
...
const create = async (req, res) => {
  try {
    ...
    await HeaderNavigation.create(data);
    return res.json({
      status: true,
      msg: "تم إنشاؤه بنجاح.",
    });
...
const update = async (req, res) => {
  try {
    ...
    await HeaderNavigation.findByIdAndUpdate(id, data);
...
const remove = async (req, res) => {
    try {
        const { id } = req.params;
        await HeaderNavigation.findByIdAndDelete(id);
