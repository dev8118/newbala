...
const EditFooterMenuDialog = props => {
  const { open, categories, navigation, onSuccess, onClose } = props
  const [loading, setLoading] = useState(false)
  const [label, setLabel] = useState('الأقسام الرئيسة')
  const [name, setName] = useState('')
  const [type, setType] = useState('static') // static | category
  const [isExternal, setIsExternal] = useState(false)
  const [category, setCategory] = useState(navigation.category ? navigation.category : '')
  const [page, setPage] = useState('home')
  const [url, setUrl] = useState('')

  useEffect(() => {
    if (open) {
      setLabel(navigation.label)
      setName(navigation.name)
      setType(navigation.type)
      setCategory(navigation.category ? navigation.category : '')
      setPage(navigation.page ? navigation.page : '')
      setUrl(navigation.url ? navigation.url : '')
    }
  }, [open])
  ...
  const handleUpdateMenu = async () => {
    if (name.trim() === '') {
      ...
      try {
        ...
        const { data } = await Http.put(`/admin/footer-navigations/${id}`, {
          label,
          name,
          type,
...
export default EditFooterMenuDialog
