...
const EditPostDialog = props => {
  ...
  const { open, categories, post, onSuccess, onClose } = props
  ...
  useEffect(() => {
    if (open) {
      ...
      setImage({
        file: null,
        url: `/uploads/posts/${post.image}`
      })
      setTitle(post.title)
      setContent(EditorState.createWithContent(ContentState.createFromBlockArray(convertFromHTML(post.content))))

      const _galleries = post.gallery.map(gallery => {
        return {
          url: `/uploads/posts/${gallery}`,
          file: null
        }
      })
    ...
    if (title.trim() === '') {
      ...
      try {
        ...
        for (let gallery of galleries) {
          formData.append(
            'gallery',
            gallery.file ? gallery.file : gallery.url.split('/')[gallery.url.split('/').length - 1]
          )
        }
        const id = post._id
        const { data } = await Http.put(`/admin/posts/${id}`, formData)
        if (data.status) {
          ...
          <Grid item md={6} xs={12}>
            <DropzoneWrapper sx={{ mb: 5 }}>
              <Box {...getRootProps({ className: 'dropzone' })}>
                <input {...getInputProps()} />
                {image ? (
                  <Box component='img' key={image.url} alt={image.url} className='single-file-image' src={image.url} />
                ) : (
